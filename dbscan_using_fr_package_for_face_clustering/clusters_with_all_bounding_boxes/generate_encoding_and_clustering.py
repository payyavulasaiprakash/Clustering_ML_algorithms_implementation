...
des_path=args['o']
cluster_folder_path=input_folder_path+"_cluster_with_bbox"
...
def generate_encodings_for_images(input_folder_path):
    ...
    for image_path in input_images:
        ...
        face_encodings=face_recognition.face_encodings(image_rgb,bounding_boxes)  #encodings, outputs the image in bgr format
        print(bounding_boxes)
        #if i take all the bounding boxes in to account while clustering then the same image will be in different clusters due to different bounding boxes
        for bbox_no in range(len(bounding_boxes)): 
            image_output=cv2.rectangle(image_rgb,(bounding_boxes[bbox_no][3],bounding_boxes[bbox_no][0]),(bounding_boxes[bbox_no][1],bounding_boxes[bbox_no][2]),color = (0, 255, 0),thickness = 2)
